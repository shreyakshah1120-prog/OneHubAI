...
class Chat(db.Model):
    """A conversation thread. `original_prompt` / `enhanced_prompt` / `response`
    are kept (and populated from the first turn) for backward compatibility
    with older code/templates that read them directly; the full multi-turn
    history lives in ChatMessage."""
    __tablename__ = "chats"
    id = db.Column(db.Integer, primary_key=True)
...
class ChatMessage(db.Model):
    """A single turn inside a Chat thread."""
