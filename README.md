...
# 4) configure
# Create a file named .env beside run.py and requirements.txt
...
Create a `.env` file in the project root, beside `run.py` and `requirements.txt`. The only **required** value is `SECRET_KEY`. For the Health AI module, add `GEMINI_API_KEY`.
